...
const Speakers = () => {
    return (
        <section className="max-container flex flex-col md:gap-4 py-4 px-2 nav-link-outline rounded-3xl m-6 mt-2 items-center justify-center">
            ...
            {SPEAKERS.map((speaker, index) => (
                <div key={index} className="relative mt-1 bg-[#1d1d1c] lg:hover:bg-[#d4e20b] w-full rounded-2xl flex flex-row text-lg lg:text-3xl hover:text-black items-center group">
                    ...
                    <div className="p-2 md:p-8 flex-grow lg:group-hover:text-black">
                        {speaker.name}
                    </div>
                    <div className="p-2 md:p-8 text-sm md:text-xl text-start ml-auto lg:group-hover:text-black">
                        {speaker.company}
                    </div>
                </div>
            ))}
